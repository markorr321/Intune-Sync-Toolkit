# ============================================================
# Sync-AllMacOSDevices.ps1
# ============================================================
#
# DESCRIPTION:
# This script automates the process of syncing ALL macOS devices in Microsoft Intune.
# It connects to Microsoft Graph, retrieves all macOS managed devices in the tenant,
# and triggers a sync operation on each device. Perfect for bulk policy updates
# or maintenance operations across all Mac devices.
#
# AUTHOR: Mark Orr
...
#
# KEY FEATURES:
# - Automatically finds all macOS devices in Intune
# - Triggers sync on all discovered Mac devices
...
#
# TYPICAL USE CASES:
# - Force sync all Mac devices after policy changes
# - Bulk maintenance operations on macOS fleet
# - Ensure all Macs get latest security updates
# - Troubleshoot macOS device compliance across tenant
#
# REQUIREMENTS:
...
# USAGE EXAMPLES:
# 
# # Sync all macOS devices in tenant
# .\Sync-AllMacOSDevices.ps1
# 
...
<#
.SYNOPSIS
    Syncs all macOS devices in Microsoft Intune

.DESCRIPTION
    This script connects to Microsoft Graph, retrieves all macOS managed devices,
...
param(
    [string]$LogPath = "C:\temp\MacOSDeviceSync.log"
)
...
function Get-AllMacOSDevices {
    Write-Log "Retrieving all macOS devices from Intune..."
    ...
    try {
        if ($PSVersionTable.PSVersion.Major -ge 7) {
            ...
            $MacDevices = $AllDevices | Where-Object { 
                $_.OperatingSystem -eq "macOS"
            }
        } else {
            Write-Log "Using Intune module for device retrieval..."
            # Get all macOS devices using the Intune module approach
            $MacDevices = Get-IntuneManagedDevice -Filter "operatingsystem eq 'macOS'" | Get-MSGraphAllPages
        }
        
        Write-Log "Found $($MacDevices.Count) macOS devices in tenant"
        
        return $MacDevices
    } catch {
        Write-Log "Error retrieving macOS devices: $_" -Level "ERROR"
        return @()
    }
}
...
# Main execution
function Main {
    Write-Log "=== macOS Device Sync Script Started ==="
    ...
    # Get all macOS devices
    $MacDevices = Get-AllMacOSDevices
    
    if ($MacDevices.Count -eq 0) {
        Write-Log "No macOS devices found in tenant." -Level "WARNING"
        return
    }
    
    # Process each device
    $TotalDevices = $MacDevices.Count
    $SyncedDevices = 0
    $FailedSyncs = 0
    $Counter = 0
    ...
    foreach ($Device in $MacDevices) {
        $Counter++
        Write-Progress -Activity "Syncing macOS Devices" -Status "Processing device $Counter of $TotalDevices" -PercentComplete (($Counter / $TotalDevices) * 100)
    ...
    Write-Progress -Activity "Syncing macOS Devices" -Completed
    
    # Summary
    Write-Log "=== SYNC SUMMARY ==="
    Write-Log "Total macOS devices found: $TotalDevices"
