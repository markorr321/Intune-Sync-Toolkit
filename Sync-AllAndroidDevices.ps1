# ============================================================
# Sync-AllAndroidDevices.ps1
# ============================================================
#
# DESCRIPTION:
# This script automates the process of syncing ALL Android devices in Microsoft Intune.
# It connects to Microsoft Graph, retrieves all Android managed devices in the tenant,
# and triggers a sync operation on each device. Perfect for bulk policy updates
# or maintenance operations across all Android devices.
#
# AUTHOR: Mark Orr
...
#
# KEY FEATURES:
# - Automatically finds all Android devices in Intune
# - Triggers sync on all discovered Android devices
...
#
# TYPICAL USE CASES:
# - Force sync all Android devices after policy changes
# - Bulk maintenance operations on Android fleet
# - Ensure all Android devices get latest security updates
# - Troubleshoot Android device compliance across tenant
#
# REQUIREMENTS:
...
# USAGE EXAMPLES:
# 
# # Sync all Android devices in tenant
# .\Sync-AllAndroidDevices.ps1
# 
# # Custom log file location
# .\Sync-AllAndroidDevices.ps1 -LogPath "C:\Logs\AndroidSync.log"
#
# ============================================================

<#
.SYNOPSIS
    Syncs all Android devices in Microsoft Intune

.DESCRIPTION
    This script connects to Microsoft Graph, retrieves all Android managed devices,
...
param(
    [string]$LogPath = "C:\temp\AndroidDeviceSync.log"
)
...
function Get-AllAndroidDevices {
    Write-Log "Retrieving all Android devices from Intune..."
    ...
    try {
        if ($PSVersionTable.PSVersion.Major -ge 7) {
            ...
            $AndroidDevices = $AllDevices | Where-Object { 
                $_.OperatingSystem -eq "Android"
            }
        } else {
            Write-Log "Using Intune module for device retrieval..."
            # Get all Android devices using the Intune module approach
            $AndroidDevices = Get-IntuneManagedDevice -Filter "operatingsystem eq 'Android'" | Get-MSGraphAllPages
        }
        
        Write-Log "Found $($AndroidDevices.Count) Android devices in tenant"
        
        return $AndroidDevices
    } catch {
        Write-Log "Error retrieving Android devices: $_" -Level "ERROR"
        return @()
    }
}
...
# Main execution
function Main {
    Write-Log "=== Android Device Sync Script Started ==="
    ...
    # Get all Android devices
    $AndroidDevices = Get-AllAndroidDevices
    
    if ($AndroidDevices.Count -eq 0) {
        Write-Log "No Android devices found in tenant." -Level "WARNING"
        return
    }
    ...
    foreach ($Device in $AndroidDevices) {
        $Counter++
        Write-Progress -Activity "Syncing Android Devices" -Status "Processing device $Counter of $TotalDevices" -PercentComplete (($Counter / $TotalDevices) * 100)
    ...
    Write-Progress -Activity "Syncing Android Devices" -Completed
    
    # Summary
    Write-Log "=== SYNC SUMMARY ==="
    Write-Log "Total Android devices found: $TotalDevices"
